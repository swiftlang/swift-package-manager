...
var includeDynamicLibrary: Bool {
#if os(Windows)
    false
#else
    true
...
@MainActor
func LLBuildDependency<T>(_ dependencies: [T] = []) -> [T] {
    if useLLBuildFramework { return [] }
    return dependencies
}

@MainActor
func SwiftBuildDependency<T>(_ dependencies: [T] = []) -> [T] {
    if useSwiftBuildFramework { return [] }
    return dependencies
}
...
var products: [Product] {
    ...
    /**
     SwiftPMDataModel is the subset of SwiftPM product that includes just its data
     model.  This allows some clients (such as IDEs) that use SwiftPM's data model
     but not its build system to not have to depend on SwiftDriver, SwiftLLBuild,
     etc. We should probably have better names here, though that could break some
     clients.
    **/
    ...
    /**
     ...
     packages.  `libSwiftPM` includes all of the SwiftPM code except the command
     line tools, while `libSwiftPMDataModel` includes only the data model.
    ...
    **/
    ...
    return [swiftPMDataModelProduct, swiftPMProduct].flatMap {
        [
            .library(name: "\($0.name)-auto", targets: $0.targets),
...
var targets: [Target] {
    var _targets: [Target] = [
        ...
        .target(
            ...
            name: "SPMLLBuild",
            dependencies: ["Basics"] + LLBuildDependency([
    ...
    // rdar://101868275 "error: cannot find 'XCTAssertEqual' in scope" can
    // affect almost any functional test, so we flat out disable them all until
    // we know what is going on
    if !defined("SWIFTCI_DISABLE_SDK_DEPENDENT_TESTS") {
        _targets.append(contentsOf: [
            .testTarget(
    ...
    if buildOnlyTests {
        return _targets.compactMap { target in
            guard target.isTest || target.name.contains("TestSupport") else {
                return nil
            }

            if [
                // PackageCollectionModel is not built in CMake
                "PackageCollectionModelTests",
                "PackageCollectionsModelTests",
                // QueryEngine is not built in CMake
                "QueryEngineTests",
                // LLBuildManifest needs to be built for testable import
                "LLBuildManifestTests",
                // PackageFingerprint needs to be built for testable import
                "PackageFingerprintTests",
                // SourceKitLSPAPI needs to be built for testable import
                "SourceKitLSPAPITests",
                // SPMBuildCore needs to be built for testable import
                "SPMBuildCoreTests",
                // PackageCollectionsSigning needs to be built for testable import
                "PackageCollectionsSigningTests",
                // PackageModel needs to be built for testable import
                "PackageModelTests",
                // SwiftFixIt needs to be built for testable import
                "SwiftFixItTests",
                // PackageGraph needs to be built for testable import
                "FunctionalTests",
                "PackageGraphTests",
                // SourceControl needs to be built for testable import
                "SourceControlTests",
                // PackageSigning needs to be built for testable import
                "PackageSigningTests",
                // Commands needs to be built for testable import
                "CommandsTests",
                // Basics needs to be built for testable import
                "PackageLoadingTests",
                "WorkspaceTests",
                "BasicsTests",
                // PackageModel needs to be built for testable import
                "XCBuildSupportTests",
                // Build needs to be built for testable import
                "BuildTests",
                // PackageRegistry needs to be built for testable import
                "PackageRegistryTests",
                // PackageCollections needs to be built for testable import
            ...
            target.dependencies = target.dependencies.filter { dependency in
                if case .byNameItem(name: let name, _) = dependency,
