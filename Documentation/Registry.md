...
- [4. Endpoints](#4-endpoints)
  ...
  - [4.5. Publish a new package release](#45-publish-a-new-package-release)
    - [4.5.1. Synchronous and asynchronous publication](#451-synchronous-and-asynchronous-publication)
    - [4.5.2. Package release metadata](#452-package-release-metadata)
    - [4.5.3. Source repository location](#453-source-repository-location)
    - [4.5.4. Source revision](#454-source-revision)
    - [4.5.5. Package location](#455-package-location)
...
| [\[5\]](#endpoint-5) | `PUT`  | `/{package}/{version}{?commit,branch,tag,path,url}`  | Publish a package release                     |
...
### 4.5. Publish a new package release
...
```http
PUT /github.com/mona/LinkedList/2.0.0 HTTP/1.1
Host: packages.example.com
...
A server SHALL use the `swift package archive` tool
to create a source archive for the release.
A server SHALL use the `swift package compute-checksum` tool
to compute the checksum used to verify the contents of a source archive.
Once a release has been created,
its associated source archive MUST NOT change.
A registry MAY remove a release after its published.

#### 4.5.1. Synchronous and asynchronous publication

If no release currently exists for the requested version,
a server SHOULD respond quickly.
A server SHOULD attempt to access the source of the package release
before sending a response,
so that it may communicate any errors to the client immediately.
A server MAY set a timeout to guarantee a timely response to each request.

A server MAY respond to a request to publish a new package release
either synchronously or asynchronously.
A client MAY send indicate their preference for asynchronous processing
...
the server SHALL respond with a status code of `202` (Accepted)
to acknowledge that the request is being processed.
This response SHOULD also contain
a `Location` header with a URL that the client can poll for progress updates and
a `Retry-After` header with an estimate of when processing is expected to finish.
...
the server SHOULD respond with a status code of `204` (No Content) and
a `Retry-After` header with an estimate of when processing is expected to finish.
...
the server SHOULD respond with a status code of `205` (Reset Content).
...
the server SHALL respond with a status code of `409` (Conflict)
...
by responding with a status code of `403` (Forbidden).

#### 4.5.2. Package release metadata
...
If a client sends an invalid JSON document in its request body,
...
If a client sends a request to publish a package release that already exists,
a server SHOULD update the metadata for that release
with any additional information in its request body
and respond with a status code of `204` (No Content).
...
A server SHOULD NOT partially apply updates to metadata.
For example,
if an existing release has the metadata `{"a": true}`
and a client sends a request to update its metadata to `{"b": false}`,
the resulting metadata for this release should be `{"b": false}`
and not `{"a": true, "b": false}`.

A client MAY send an empty JSON document (`{}`)
to request the server to clear any metadata for an existing release.
A server SHALL NOT update the metadata for an existing release
in response to a request with an empty request body.

#### 4.5.3. Source repository location

A server SHOULD determine the location of a package's source repository
from the requested URI, if possible.

For example,
if a server hosts source code on the `example.com` domain
and hosts a package registry service on the `packages.example.com` subdomain,
a `PUT` request to `packages.example.com/github.com/mona/LinkedList/1.1.1`
would be determined to correspond to
the source code at `example.com/github.com/mona/LinkedList` at the revision tagged `1.1.1`.

A client MAY specify an explicit location
with a `url` query item parameter.

```http
PUT /github.com/mona/LinkedList/1.1.1?url=https://example.org/mona/LinkedList.git HTTP/1.1
Accept: application/vnd.swift.registry.v1
```
...
it SHOULD respond with `502` (Bad Gateway)
...
#### 4.5.4. Source revision

The `version` path parameter in a client's request
...
with any combination of `commit`, `tag`, and `branch` query item parameters.

```http
PUT /github.com/mona/LinkedList/1.1.1?tag=v1.1.1 HTTP/1.1
Host: packages.example.com
Accept: application/vnd.swift.registry.v1
```
...
with the provided `commit`, `tag`, or `branch` parameters.
...
A server SHOULD respond with a status code of `409` (Conflict)
if any provided `commit`, `tag`, or `branch` parameters
...
A server SHOULD support Git repositories.
A server MAY support other source control systems,
such as Mercurial or Subversion.

#### 4.5.5. Package location

A client MAY use `path` and `url` parameters together
...
If a client specifies a `path` parameter,
a server SHOULD look for a `Package.swift` file at that location.
Otherwise, it SHOULD look at the root directory.
If a `Package.swift` file doesn't exist at that location,
a server SHOULD respond with a status code of `404` (Not Found).
