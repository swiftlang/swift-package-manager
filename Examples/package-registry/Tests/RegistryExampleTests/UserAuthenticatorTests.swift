...
struct UserAuthenticatorTests {
    ...
    @Test func `correct email and password authenticate`() async throws {
        ...
        #expect(await auth.authenticate(email: "mona@example.com", password: "hunter2"))
    }
    ...
    @Test func `wrong password fails`() async throws {
        ...
        #expect(await auth.authenticate(email: "mona@example.com", password: "wrong") == false)
    }

    @Test func `unknown email fails`() async throws {
        let auth = try await seededAuthenticator { _ in }
        #expect(await auth.authenticate(email: "ghost@example.com", password: "hunter2") == false)
    }
    ...
    @Test func `empty password fails`() async throws {
        ...
        #expect(await auth.authenticate(email: "mona@example.com", password: "") == false)
    }
    ...
    @Test func `correct token authenticates`() async throws {
        ...
        #expect(await auth.authenticate(token: "the-token"))
    }
    ...
    @Test func `unknown token fails`() async throws {
        ...
        #expect(await auth.authenticate(token: "some-other-token") == false)
    }
    ...
    @Test func `empty token fails`() async throws {
        ...
        #expect(await auth.authenticate(token: "") == false)
    }
    ...
    @Test func `token user cannot authenticate via basic with the plaintext token`() async throws {
        ...
        #expect(await auth.authenticate(email: "mona@example.com", password: "the-token") == false)
    }
    ...
    @Test func `token user cannot authenticate via basic with the token hash`() async throws {
        ...
        #expect(await auth.authenticate(email: "mona@example.com", password: hash) == false)
    }
    ...
    @Test func `password user cannot authenticate via bearer`() async throws {
        ...
        #expect(await auth.authenticate(token: "hunter2") == false)
    }
}
