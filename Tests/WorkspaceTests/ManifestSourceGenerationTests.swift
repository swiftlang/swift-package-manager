...
class ManifestSourceGenerationTests: XCTestCase {
    ...
    private func testManifestWritingRoundTrip(manifestContents: String, toolsVersion: ToolsVersion, fs: FileSystem = localFileSystem) throws {
        try withTemporaryDirectory { packageDir in
            ...
            let manifest = try manifestLoader.load(at: packageDir, packageKind: .root, packageLocation: packageDir.pathString, toolsVersion: toolsVersion, fileSystem: fs)
            ...
            print(newContents)
            let newManifest = try manifestLoader.load(at: packageDir, packageKind: .root, packageLocation: packageDir.pathString, toolsVersion: toolsVersion, fileSystem: fs)
