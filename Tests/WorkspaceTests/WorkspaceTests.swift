...
final class WorkspaceTests: XCTestCase {
    ...
    func testRootAsDependency1() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            XCTAssertNoMatch(workspace.delegate.events, [.equal("fetching package: \(tmpDir)/pkgs/Baz")])
    ...
    func testUpdateDryRun() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkUpdateDryRun(roots: ["Root"]) { changes, diagnostics in
                ...
                let path = tmpDir.appending(components: ["pkgs","Foo"])
                let expectedChange = (
    ...
    func testBranchAndRevision() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let bar = RepositorySpecifier(path: "\(tmpDir)/pkgs/Bar")
    ...
    func testEditDependency() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let barPath = tmpDir.appending(components: ["custom", "bar"])
    ...
    func testUnsafeFlagsInEditedPackage() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let editedFooPath = tmpDir.appending("Foo")
    ...
    func testLocalArchivedArtifactExtractionHappyPath() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "A", "A1.xcframework"])))
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "A", "A2", "artifactbundle"])))
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "B", "B", "xcframework"])))

            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                XCTAssertNoDiagnostics(diagnostics)

                // Ensure that the artifacts have been properly extracted
                XCTAssertTrue(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "a", "A1", "A1.xcframework"])))
                XCTAssertTrue(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "a", "A2", "A2.artifactbundle"])))
                XCTAssertTrue(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "b", "B", "B.xcframework"])))
                ...
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract", "a", "A1"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract", "a", "A2"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract", "b", "B"]),
                ])

                // Ensure that the temporary directories have been removed
                XCTAssertTrue(try! fs.getDirectoryContents(tmpDir.appending(components: [".build", "artifacts", "extract", "a", "A1"])).isEmpty)
                XCTAssertTrue(try! fs.getDirectoryContents(tmpDir.appending(components: [".build", "artifacts", "extract", "a", "A2"])).isEmpty)
                XCTAssertTrue(try! fs.getDirectoryContents(tmpDir.appending(components: [".build", "artifacts", "extract", "b", "B"])).isEmpty)
            }
    ...
    func testLocalArchivedArtifactSourceTransitionPermutations() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                ...
                XCTAssertTrue(try fs.exists(AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A1/\(a1FrameworkName)")))
                XCTAssertTrue(
                    try fs
                        .exists(
                            AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A3/\(a3FrameworkName)/local-archived")
                        )
                )
                XCTAssertTrue(
                    try fs
                        .exists(AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A4/\(a4FrameworkName)/remote"))
                )

                // Ensure that the old artifacts have been removed
                XCTAssertFalse(try fs.exists(AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A2/\(a2FrameworkName)")))
                XCTAssertFalse(
                    try fs
                        .exists(AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A3/\(a3FrameworkName)/remote"))
                )
                XCTAssertFalse(
                    try fs
                        .exists(
                            AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A4/\(a4FrameworkName)/local-archived")
                        )
                )
                XCTAssertFalse(try fs.exists(AbsolutePath(validating: "\(tmpDir)/.build/artifacts/a/A5/\(a5FrameworkName)")))
            }
...
////// STAET ATDIN
    func testLocalArchivedArtifactChecksumChange() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, _ in
                ...
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "A1"]),
                ])
            }
    ...
    func testLocalArchivedArtifactStripFirstComponent() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            // ensure that the artifacts do not exist yet
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "root", "flat", "flat.xcframework"])))
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "root", "nested", "nested.artifactbundle"])))
            XCTAssertFalse(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "root", "nested2", "nested2.xcframework"])))

            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                XCTAssertNoDiagnostics(diagnostics)
                XCTAssert(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "root"])))
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "flat"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "nested"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "nested2"]),
                ])
            }
        ...
        try await withTemporaryDirectory { tmpDir in
            ...
            await workspace.checkPackageGraphFailure(roots: ["Root"]) { diagnostics in
                testDiagnostics(diagnostics) { result in
                    result.checkUnordered(
                        diagnostic: .contains(
                            "local binary target 'A1' at '\(tmpDir.appending(components: ["roots", "Root", "XCFrameworks", "incorrect.xcframework"]))' does not contain a binary artifact."
                        ),
                        severity: .error
                    )
                    result.checkUnordered(
                        diagnostic: .contains(
                            "local binary target 'A2' at '\(tmpDir.appending(components: ["roots", "Root", "ArtifactBundles", "incorrect.artifactbundle"]))' does not contain a binary artifact."
                        ),
                        severity: .error
                    )
                }
    ...
    func testArtifactDownloadHappyPath() async throws {
        ...
        let fs = InMemoryFileSystem()
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                XCTAssertNoDiagnostics(diagnostics)
                XCTAssert(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "a"])))
                XCTAssert(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "b"])))
    ...
    func testArtifactDownloadWithPreviousState() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            await workspace.checkPackageGraphFailure(roots: ["Root"]) { diagnostics in
                ...
                XCTAssert(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "b"])))
                XCTAssert(fs.exists(tmpDir.appending(components: [".build", "artifacts", "a", "A1", "A1.xcframework"])))
                XCTAssert(fs.exists(tmpDir.appending(components: [".build", "artifacts", "a", "A2", "A2.xcframework"])))
                XCTAssert(!fs.exists(tmpDir.appending(components: [".build", "artifacts", "a", "A3", "A3.xcframework"])))
                XCTAssert(!fs.exists(tmpDir.appending(components: [".build", "artifacts", "a", "A4", "A4.xcframework"])))
                XCTAssert(!fs.exists(tmpDir.appending(components: [".build", "artifacts", "a", "A5", "A5.xcframework"])))
                XCTAssert(fs.exists(tmpDir.appending(components: ["pkgs", "a", "XCFrameworks", "A7.xcframework"])))
                XCTAssert(!fs.exists(tmpDir.appending(components: [".build", "artifacts", "Foo"])))
                XCTAssertEqual(downloads.map(\.key.absoluteString).sorted(), [
                    "https://a.com/a2.zip",
                    "https://a.com/a3.zip",
                    "https://a.com/a7.zip",
                ...
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract","a", "A2"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract","a", "A3"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract","a", "A7"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract","b", "B"]),
                ])
                XCTAssertEqual(
                    downloads.map(\.value).sorted(),
    ...
    func testArtifactDownloadServerError() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            XCTAssertTrue(fs.isDirectory(tmpDir.appending(components: [".build", "artifacts", "root"])))
            XCTAssertFalse(fs.exists(tmpDir.appending(components: [".build", "artifacts", "root", "a.zip"])))
    ...
    func testArtifactDownloaderOrArchiverError() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let archiver = MockArchiver(handler: { _, _, destinationPath, completion in
                XCTAssertEqual(destinationPath.parentDirectory, tmpDir.appending(components: [".build", "artifacts", "extract", "root", "A2"]))
                completion(.failure(DummyError()))
            })
    ...
    func testDownloadedArtifactTransitive() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                ...
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract", "a", "A"]),
                ])
                XCTAssertEqual(
                    downloads.map(\.value).sorted(),
                    archiver.extractions.map(\.archivePath).sorted()
    ...
    func testArtifactMultipleExtensions() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                ...
                XCTAssertEqual(archiver.extractions.map(\.destinationPath.parentDirectory).sorted(), [
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "A1"]),
                    tmpDir.appending(components: [".build", "artifacts", "extract", "root", "A2"]),
                ])
                XCTAssertEqual(
                    downloads.map(\.value).sorted(),
                    archiver.extractions.map(\.archivePath).sorted()
    ...
    func testDuplicateDependencyIdentityWithNameAtRoot() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                testDiagnostics(diagnostics) { result in
                    result.check(
                        diagnostic: "Conflicting identity for utility: dependency '\(CanonicalPackageLocation(tmpDir.pathString))/pkgs/bar/utility' and dependency '\(CanonicalPackageLocation(tmpDir.pathString))/pkgs/foo/utility' both point to the same package identity 'utility'.",
                        severity: .error
                    )
                }
            }
    ...
    func testDuplicateTransitiveIdentityMultiplePossibleChains() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let temperedGlassPath = tmpDir.appending(components: ["pkgs", "tempered", "glass"])
            let standardGlassPath = tmpDir.appending(components: ["pkgs", "standard", "glass"])
            let tmpDirCanonicalPackageLocation = CanonicalPackageLocation(tmpDir.pathString)
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                testDiagnostics(diagnostics) { result in
                    let prefix1 = CanonicalPackageLocation(temperedGlassPath.pathString)
                    let prefix2 = CanonicalPackageLocation(standardGlassPath.pathString)
                    result.check(
                        diagnostic: "Conflicting identity for glass: dependency '\(prefix1)' and dependency '\(prefix2)' both point to the same package identity 'glass'. The dependencies are introduced through the following chains: (A) \(tmpDirCanonicalPackageLocation)/roots/root->\(tmpDirCanonicalPackageLocation)/pkgs/house->\(tmpDirCanonicalPackageLocation)/pkgs/premium_window->\(tmpDirCanonicalPackageLocation)/pkgs/tempered/glass (B) \(tmpDirCanonicalPackageLocation)/roots/root->\(tmpDirCanonicalPackageLocation)/pkgs/shack->\(tmpDirCanonicalPackageLocation)/pkgs/standard/glass. If there are multiple chains that lead to the same dependency, only the first chain is shown here. To see all chains use debug output option. To resolve the conflict, coordinate with the maintainer of the package that introduces the conflicting dependency.",
                        severity: .error
                    )
                }
            }
            try await workspace.checkPackageGraph(roots: ["Root"]) { _, diagnostics in
                testPartialDiagnostics(diagnostics, minSeverity: .debug) { result in
                    result.checkUnordered(
                        diagnostic: .contains("Conflicting identity for glass: chains of dependencies for \(temperedGlassPath): [[\(tmpDirCanonicalPackageLocation)/roots/root, \(tmpDirCanonicalPackageLocation)/pkgs/house, \(tmpDirCanonicalPackageLocation)/pkgs/premium_window, \(tmpDirCanonicalPackageLocation)/pkgs/tempered/glass]]"),
                        severity: .debug
                    )
                    result.checkUnordered(
                        diagnostic: .contains("Conflicting identity for glass: chains of dependencies for \(standardGlassPath): [[\(tmpDirCanonicalPackageLocation)/roots/root, \(tmpDirCanonicalPackageLocation)/pkgs/shack, \(tmpDirCanonicalPackageLocation)/pkgs/standard/glass], [\(tmpDirCanonicalPackageLocation)/roots/root, \(tmpDirCanonicalPackageLocation)/pkgs/house, \(tmpDirCanonicalPackageLocation)/pkgs/budget_window, \(tmpDirCanonicalPackageLocation)/pkgs/standard/glass]]"),
                        severity: .debug
                    )
                }
            }
        ...
        try await withTemporaryDirectory { tmpDir in
            ...
            try await workspace.checkPackageGraph(roots: ["River", "Lake"]) { _, diagnostics in
                testPartialDiagnostics(diagnostics, minSeverity: .debug) { result in
                    ...
                    let tmpDirCanonicalPackageLocation = CanonicalPackageLocation(tmpDir.pathString)
                    let prefix = tmpDir.appending(components: ["pkgs", "standing", "water"])
                    result.checkUnordered(
                    ...
                    let prefix2 = tmpDir.appending(components: ["pkgs", "flowing", "water"])
                    result.checkUnordered(
    ...
    func testCustomPackageContainerProvider() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let fooPath = tmpDir.appending("Foo")
