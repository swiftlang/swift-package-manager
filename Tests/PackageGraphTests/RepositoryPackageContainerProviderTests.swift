...
class RepositoryPackageContainerProviderTests: XCTestCase {
    ...
    func testVersions() throws {
            ...
            let filePath = repoPath.appending(component: ".swift-version")
            ...
            try repo.writeFileContents(filePath, bytes: "3.1.0")
            repo.commit()
            ...
            try repo.writeFileContents(filePath, bytes: "4.0.0")
            repo.commit()
            ...
            do {
                let provider = createProvider(try ToolsVersion(string: "3.1.0"))
            ...
            do {
                let provider = createProvider(try ToolsVersion(string: "4.0.0"))
                let container = try await { provider.getContainer(for: specifier, completion: $0) }
                let v = container.versions.map{$0}
            ...
            do {
                let provider = createProvider(try ToolsVersion(string: "3.0.0"))
                let container = try await { provider.getContainer(for: specifier, completion: $0) }
                let v = container.versions.map{$0}
                XCTAssertEqual(v, [])
            }
