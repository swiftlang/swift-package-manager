...
final class AuthorizationProviderTests: XCTestCase {
    ...
    func testKeychain_protocol() throws {
        ...
        // Delete
        XCTAssertNoThrow(try tsc_await { callback in provider.remove(for: httpURL, callback: callback) })
        XCTAssertNil(provider.authentication(for: httpURL))
        self.assertAuthentication(provider, for: httpsURL, expected: (user, newHTTPSPassword))

        XCTAssertNoThrow(try tsc_await { callback in provider.remove(for: httpsURL, callback: callback) })
    ...
    func testKeychain_port() throws {
        ...
        // Delete
        XCTAssertNoThrow(try tsc_await { callback in provider.remove(for: noPortURL, callback: callback) })
        XCTAssertNil(provider.authentication(for: noPortURL))
        self.assertAuthentication(provider, for: portURL, expected: (user, newPortPassword))

        XCTAssertNoThrow(try tsc_await { callback in provider.remove(for: portURL, callback: callback) })
