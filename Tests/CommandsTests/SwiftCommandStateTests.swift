...
final class SwiftCommandStateTests: CommandsTestCase {
    ...
    func testToolchainOption() async throws {
        try XCTSkipOnWindows(because: #"threw error \"toolchain is invalid: could not find CLI tool `swiftc` at any of these directories: [<AbsolutePath:\"\usr\bin\">]\", needs investigation"#)
