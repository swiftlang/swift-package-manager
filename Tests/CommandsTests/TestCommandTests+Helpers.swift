...
import enum Commands.CoverageFormatError
import struct Basics.AbsolutePath
import struct Basics.RelativePath
import func Commands.getHtmlCoverageOutputDir
...
struct TestCommmandHelpersTests {
    ...
    @Suite
    struct getHtmlCoverageOutputDirTests {
        @Test(
            arguments: [
                "",
                """
        ...
        func outputDirArgumentNotPresentReturnsTheDefaultPath(
            content: String
        ) async throws {
            let defaultPath = AbsolutePath.root.appending("not-used")
            let contentArray = content.components(separatedBy: .newlines)
            let actual = getHtmlCoverageOutputDir(
                from: contentArray,
        ...
        struct GetHtmlCoverageOutputDirTestData: Identifiable {
            let content: String
        ...
        @Test(
            arguments: [
                ...
                return [
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)./relative/path
                            """,
                        workspacePath: AbsolutePath("/foo/bar"),
                        expected: AbsolutePath("/foo/bar/relative/path"),
                        id: "Relative Path: Single argument with seperator '\(sep)' with output directory having './' prefix",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)relative/path
                        ...
                        workspacePath: AbsolutePath("/foo/bar"),
                        expected: AbsolutePath("/foo/bar/relative/path"),
                        id: "Relative Path: Single argument with seperator '\(sep)'",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                        ...
                        id: "AbsolutePath: Single argument with seperator '\(sep)'",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)/Bar/baz
                            --output-dir\(sep)relative/path/should/win
                            """,
                        workspacePath: AbsolutePath("/foo"),
                        expected: AbsolutePath("/foo/relative/path/should/win"),
                        id: "RelativePath: Two output dir arguments with seperator '\(sep)' returns the last occurrence",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)/Bar/baz
                        ...
                        id: "AbsolutePath: Two output dir arguments with seperator '\(sep)' returns the last occurrence",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)/Bar/baz
                        ...
                        workspacePath: AbsolutePath("/foo"),
                        expected: AbsolutePath("/foo/relative/path/should/win"),
                        id: "RelativePath: three output dir arguments with seperator '\(sep)' returns the last occurrence",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)/Bar/baz
                        ...
                        workspacePath: AbsolutePath.root,
                        expected: AbsolutePath("/this/should/win"),
                        id: "AbsolutePath: three output dir arguments with seperator '\(sep)' returns the last occurrence",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            prefix
                        ...
                        workspacePath: AbsolutePath.root,
                        expected: AbsolutePath("/Bar/baz"),
                        id: "AbsolutePath: seperator '\(sep)': with content prefix",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            --output-dir\(sep)/Bar/baz
                        ...
                        workspacePath: AbsolutePath.root,
                        expected: AbsolutePath("/Bar/baz"),
                        id: "AbsolutePath: seperator '\(sep)': with content suffix",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            line_prefix
                        ...
                        workspacePath: AbsolutePath.root,
                        expected: AbsolutePath("/Bar/baz"),
                        id: "AbsolutePath: seperator '\(sep)': with line content and suffix",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            ...
                            """,
                        workspacePath: AbsolutePath.root,
                        expected: nil,  // Set to `nil` to have the test expect the default path be returned
                        id: "AbsolutePath: seperator '\(sep)': with line prefix (no space) returns the default path",
                    ),
                    GetHtmlCoverageOutputDirTestData(
                        content: """
                            prefix --output-dir\(sep)/Bar/baz
                        ...
                        workspacePath: AbsolutePath.root,
                        expected: AbsolutePath("/Bar/baz"),
                        id: "AbsolutePath: seperator '\(sep)': with line prefix (which contains a space)",
                    ),
                ]
            }.flatMap { $0 },
        )
        func contentContainsOutputDirectoryReturnsCorrectPath(
            data: GetHtmlCoverageOutputDirTestData,
        ) async throws {
            let defaultPath = AbsolutePath.root.appending("not-used")
            let actual = getHtmlCoverageOutputDir(
            ...
            let expected = data.expected ?? defaultPath
            #expect(
                actual == expected,
                "Actual (\(actual)) is not as expected (\(expected))",
            )
        }
        ...
        func contentContainsOutputDirectoryAsRelativePathReturnsCorrectPath(
            ...
            let actual = getHtmlCoverageOutputDir(
                ...
                defaultPath: AbsolutePath.root.appending("not-used"),
            )
    ...
    struct CoverageFormatOutputTests {
        ...
        func addFormatDuplicateThrowsError() async throws {
            ...
            #expect(throws: CoverageFormatError.duplicateEntry(format: .json)) {
        ...
        func getPathNonExistentFormatThrowsError() async throws {
            ...
            #expect(throws: CoverageFormatError.missingOutputPath(format: .json)) {
                try output.getPath(for: CoverageFormat.json)
            }

            #expect(throws: CoverageFormatError.missingOutputPath(format: .html)) {
        ...
        func encodeAsJSONSingle() throws {
            ...
            let _ = String(decoding: jsonData, as: UTF8.self)
