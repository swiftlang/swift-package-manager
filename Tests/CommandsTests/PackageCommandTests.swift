...
class PackageCommandTestCase: CommandsBuildProviderTestCase {
    ...
    func testDescribeJson() async throws {
        try XCTSkipOnWindows(because: "TSCBasic/Path.swift:969: Assertion failed, https://github.com/swiftlang/swift-package-manager/issues/8602n")
    ...
    func testCommandPluginBuildLogs() async throws {
        try XCTSkipOnWindows(because: "Unable to locate libSwiftScan. Fallback to `swift-frontend` dependency scanner invocation, needs investigation") 
    ...
    func testCommandPluginTestingCallbacks() async throws {
        try XCTSkipOnWindows(because: "filenames are too long")
        // Only run the test if the environment in which we're running actually supports Swift concurrency (which the plugin APIs require).
        try XCTSkipIf(!UserToolchain.default.supportsSwiftConcurrency(), "skipping because test environment doesn't support concurrency")
    ...
    func testPluginAPIs() async throws {
        try XCTSkipOnWindows(because: "$0.path.lastComponent in test code returns fullpaths on Windows: https://github.com/swiftlang/swift-package-manager/issues/8554") 
