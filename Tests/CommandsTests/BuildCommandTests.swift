...
struct BuildCommandTestCases {
    ...
    @Test(
        .serialized, // otherwise "error: Exited with unexpected signal code 6 or 11"
    ...
    func buildUnknownProductFailsWithAppropriateMessage(
        ...
        try await fixture(name: "Miscellaneous/MultipleExecutables") { fixturePath in
            ...
            if .native == buildSystem {
                ...
                #expect(stderr.contains(expectedErrorMessageRegex))
            }
        }
    }
    ...
    func buildUnknownTargetFailsWithAppropriateMessage(
        ...
        try await fixture(name: "Miscellaneous/MultipleExecutables") { fixturePath in
            ...
            #expect(stderr.contains(expectedErrorMessage))
        }
    }
    ...
    @Test(
        // .disableIfWorkingDirectoryUnsupported, // Only for swift build
    ...
    func buildCompleteMessage(
        ...
        try await withKnownIssue {
            ...
            buildSystem == .swiftbuild && ((ProcessInfo.hostOperatingSystem == .windows) || !workingDirectoryIsSupported())
        }
    }
    ...
    func buildStartMessage(
        ...
        try await fixture(name: "DependencyResolution/Internal/Simple") { fixturePath in
            ...
            withKnownIssue("Xcode build system does not emit the build started message.") {
                #expect(result.stdout.contains("Building for \(expectedString)"))
            } when: {
                buildSystem == .xcode
            }
        }
    ...
    @Test(
        .disableIfWorkingDirectoryUnsupported,
        arguments: SupportedBuildSystemOnPlatform,
    )
    func buildSystemDefaultSettings(
        ...
        try await withKnownIssue {
            try await fixture(name: "ValidLayouts/SingleModule/ExecutableNew") { fixturePath in
                ...
                // full command lines.
                let output: (stdout: String, stderr: String) = try await execute(["-c", "debug", "-v"], packagePath: fixturePath, buildSystem: buildSystem)
            ...
            ProcessInfo.hostOperatingSystem == .windows // possible long filename issue: needs investigation
        }
    }
    ...
    func executingTestsWithCoverageWithoutCodeBuiltWithCoverageGeneratesAFailure(
        ...
        try await withKnownIssue {
            ...
            buildSystem == .xcode || (buildSystem == .swiftbuild && .windows == ProcessInfo.hostOperatingSystem)
        }
    }
