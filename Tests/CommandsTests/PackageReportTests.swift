...
/// Pure-logic unit tests for the `swift package report` report-building and
...
/// invocation) and assert on `PackageReport` values directly. Complements the
/// end-to-end tests in `PackageCommandTests+Report.swift`.
@Suite(
    .tags(
        .TestSize.small,
...
struct PackageReportTests {

    // MARK: - buildPackageReport
    @Suite
    struct PackageReportBuilderTests {
        ...
        func report_isEmpty_whenNoDeprecatedProducts() throws {
            ...
            let report = buildPackageReport(graph: graph, includeTransitive: .off)
        ...
        func report_omitsUnreachableDeprecatedProduct_whenIncludeTransitiveIsReachable() throws {
            ...
            let report = buildPackageReport(graph: graph, includeTransitive: .reachable)
            #expect(report.deprecated.products.isEmpty)
        }
        ...
        func report_allMode_reportsDirectReachableAndUnreachable() throws {
            ...
            let report = buildPackageReport(graph: graph, includeTransitive: .all)
        ...
        func report_nonReachableMode_omitsDirectAndReachable() throws {
            ...
            let report = buildPackageReport(graph: graph, includeTransitive: .nonReachable)
    ...
    @Suite
    struct PackageReportProductTypeStringTests {

        @Test
        func productTypeString_mapsSupportedKinds() async throws {
            #expect(reportProductTypeString(.executable) == "executable")
            #expect(reportProductTypeString(.library(.automatic)) == "library")
            #expect(reportProductTypeString(.library(.static)) == "library")
            #expect(reportProductTypeString(.library(.dynamic)) == "library")
            #expect(reportProductTypeString(.plugin) == "plugin")
        }

        @Test
        func productTypeString_returnsNilForUnsupportedKinds() async throws {
            #expect(reportProductTypeString(.test) == nil)
            #expect(reportProductTypeString(.snippet) == nil)
            #expect(reportProductTypeString(.macro) == nil)
        }
    ...
    @Suite
    struct PackageReportTextRenderer {

        @Test
        func text_emptyReport_returnsExplicitMessage() async throws {
            let empty = PackageReport(deprecated: .init(products: []))
            #expect(renderPackageReportAsText(empty) == "No deprecated products found.")
        }
                ...
                Directly consumed deprecated products:
                  package 'producer':
                    library 'PaperLegacy' is unsupported
                      ...
                      Used by: MyApp
                """
            #expect(renderPackageReportAsText(report) == expected)
        }
        ...
        func text_transitiveReachableOnly_emitsOnlyReachableSection() async throws {
            ...
            let rendered = renderPackageReportAsText(report)
                ...
                Directly consumed deprecated products:
                  package 'producer':
                    library 'Old' is unsupported
                      Used by: Consumer
                """
            let actual = renderPackageReportAsText(report)
    ...
    @Suite
    struct PackageReportCodableTests {
        ...
        func report_roundTripsThroughJSON() throws {
            ...
            let decoded = try JSONDecoder().decode(PackageReport.self, from: encoded)
        ...
        func json_sortedKeysProducesByteStableOutput() throws {
            ...
            // order (a schema requirement — report-v1 says sorted keys at every
            // nesting level).
        ...
        func json_multipleEntriesArraySortIsPreserved() throws {
            ...
            // `buildPackageReport` places entries in is preserved on the wire.
            let report = PackageReport(
                deprecated: .init(products: [
                    DeprecatedProduct(
                        message: nil,
