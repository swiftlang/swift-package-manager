...
private struct BasicTests {
    ...
    func testExamplePackageDealer() throws {
        try withKnownIssue("These packages don't use the latest runtime library, which doesn't work with self-hosted builds") { 
    ...
    func testSwiftCompiler() throws {
        try withKnownIssue("'try!' expression unexpectedly raised an error: TSCBasic.Process.Error.missingExecutableProgram(program: \"which\")") {
            try withTemporaryDirectory { tempDir in
    ...
    func testSwiftPackageWithSpaces() async throws {
        try await withKnownIssue("unexpected failure matching") {
            try await withTemporaryDirectory { tempDir in
    ...
    func testSwiftRun() throws {
        try withKnownIssue ("package fails to build") {
            try withTemporaryDirectory { tempDir in
                ...
                let shOutput = try await executeSwiftRun(
