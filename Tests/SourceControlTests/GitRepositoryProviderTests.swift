...
class GitRepositoryProviderTests: XCTestCase {
    func testIsValidDirectory() throws {
        try testWithTemporaryDirectory { sandbox in
            ...
            XCTAssertTrue(provider.isValidDirectory(repositoryPath))
            ...
            XCTAssertTrue(provider.isValidDirectory(noCheckoutRepositoryPath))

            // non-git directory
            let notGitPath = sandbox.appending("test-not-git")
            XCTAssertFalse(provider.isValidDirectory(notGitPath))
