...
class GitRepositoryTests: XCTestCase {
    ...
    func testErrorMessagesIncludeExitStatusIfNoOtherDataIsAvailable() throws {
        try testWithTemporaryDirectory { path in
            ...
            let git = ShellHelperStub()
            let repo = GitRepository(git: git, path: testRepoPath)

            git.exitStatus = .terminated(code: 128)
            ...
            #if os(Windows)
            git.exitStatus = .abnormal(exception: 2)
            ...
            #else
            git.exitStatus = .signalled(signal: SIGKILL)
            ...
            git.exitStatus = .terminated(code: 128)
            git.stderrOutput = Array("An error occurred".utf8)
...
private final class ShellHelperStub: ShellHelper {
