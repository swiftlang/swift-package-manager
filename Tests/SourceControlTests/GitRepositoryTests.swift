...
class GitRepositoryTests: XCTestCase {
    ...
    func testErrorMessagesIncludeExitStatusIfNoOtherDataIsAvailable() throws {
        try testWithTemporaryDirectory { path in
            let testRepoPath = path.appending(component: "test-repo")
            try makeDirectories(testRepoPath)
            initGitRepo(testRepoPath)

            let strategy = ShellHelperStrategyStub()
            let git = GitShellHelper(strategy: strategy)
            let repo = GitRepository(git: git, path: testRepoPath)

            strategy.exitStatus = .terminated(code: 128)

            XCTAssertThrowsError(try repo.fetch(), "") { error in
                XCTAssertEqual(String(describing: error), """
                Couldn’t fetch updates from remote repositories:
                    Git process terminated with status 128
                """)
            }

            #if os(Windows)
            strategy.exitStatus = .abnormal(exception: 2)

            XCTAssertThrowsError(try repo.fetch(), "") { error in
                XCTAssertEqual(String(describing: error), """
                Couldn’t fetch updates from remote repositories:
                    Git process abnormally terminated with exception 2
                """)
            }
            #else
            strategy.exitStatus = .signalled(signal: SIGKILL)

            XCTAssertThrowsError(try repo.fetch(), "") { error in
                XCTAssertEqual(String(describing: error), """
                Couldn’t fetch updates from remote repositories:
                    Git process terminated due to signal 9
                """)
            }
            #endif

            // If stderr is not empty, the exit status is not printed.

            strategy.exitStatus = .terminated(code: 128)
            strategy.stderrOutput = Array("An error occurred".utf8)
            ...
            // Errors generated by GitRepositoryProvider should result in the same error messages as given by GitRepository. Let's test one to ensure they go through the same code path.

            strategy.exitStatus = .terminated(code: 128)
            strategy.stderrOutput = []

            let provider = GitRepositoryProvider(cancellator: Cancellator(observabilityScope: .none), git: git)
            let repoSpec = RepositorySpecifier(path: testRepoPath)
            let testCheckoutPath = path.appending(component: "checkout")

            XCTAssertThrowsError(try provider.fetch(repository: repoSpec, to: testCheckoutPath), "") { error in
                XCTAssertEqual(String(describing: error), """
                Failed to clone repository \(testRepoPath):
                    Git process terminated with status 128
                """)
            }
        }
...
private final class ShellHelperStrategyStub: ShellHelperStrategy {
    var exitStatus: ProcessResult.ExitStatus = .terminated(code: 0)
    var stderrOutput: [UInt8] = []
    ...
    func run(_ args: [String], environment: EnvironmentVariables, outputRedirection: TSCBasic.Process.OutputRedirection) throws -> String {
        let result = ProcessResult(arguments: [],
                                   environment: [:],
                                   exitStatus: try XCTUnwrap(exitStatus),
                                   output: .success([]),
                                   stderrOutput: .success(try XCTUnwrap(stderrOutput)))
