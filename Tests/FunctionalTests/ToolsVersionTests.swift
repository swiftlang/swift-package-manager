...
class ToolsVersionTests: XCTestCase {
    ...
    func testToolsVersion() throws {
        mktmpdir { path in
            ...
            XCTAssertEqual(try popen([exe]).chomp(), "foo@1.0")
            ...
            do {
                ...
                XCTAssertTrue(output.hasPrefix("swift-build: error: Package requires minimum Swift tools version 10000.1.0."))
            }
            ...
            do {
                ...
                XCTAssertTrue(output.hasPrefix("swift-build: error: Target Primary's sources are compatible with compiler version(s): 1000"))
            }
