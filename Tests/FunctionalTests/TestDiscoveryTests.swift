...
struct TestDiscoveryTests {
    ...
    // Conditional parameters to tests are not supported by Swift Testing currently, so we construct the list of params here.
    static var buildSystems: [BuildSystemProvider.Kind] = {
        if ProcessInfo.hostOperatingSystem == .windows {
            ...
            return [BuildSystemProvider.Kind.native, .swiftbuild]
        }
    }()
