...
struct MiscellaneousTestCase {
    ...
    func externalDependencyEdges1(
        ...
        try await fixture(name: "DependencyResolution/External/Complex") { fixturePath in
            ...
            (ProcessInfo.hostOperatingSystem == .windows && data.buildSystem == .native && data.config == .debug)
    ...
    func pkgConfigCFamilyTargets(
        ...
        try await fixture(name: "Miscellaneous/PkgConfig") { fixturePath in
            ...
            (ProcessInfo.hostOperatingSystem == .windows && data.buildSystem == .native)
            || (ProcessInfo.hostOperatingSystem == .windows && data.buildSystem == .swiftbuild && data.config == .debug)
        }
    }
    ...
    func noJSONOutputWithFlatPackageStructure(
        ...
        try await fixture(name: "Miscellaneous/FlatPackage") { packagePath in
            ...
            let buildOutput = try packagePath.appending(components: data.buildSystem.binPath(for: data.config))
            ...
            let testOutput = try packagePath.appending(components: data.buildSystem.binPath(for: data.config))
    ...
    func noWarningFromRemoteDependencies(
        ...
        try await fixture(name: "Miscellaneous/DependenciesWarnings") { path in
            ...
            let buildOutput = try appPath.appending(components: data.buildSystem.binPath(for: data.config))
            expectDirectoryExists(at: buildOutput)
