...
final class BuildPlanTests: XCTestCase {
    ...
    func testSwiftBundleUnavailableAccessor() throws {
        ...
        let fooResourceAccessor = fooTarget.sources.first{ $0.basename == "resource_bundle_accessor.swift" }!
        let fooContents = try fs.readFileContents(fooResourceAccessor).cString
        XCTAssertTrue(fooContents.contains("extension Foundation.Bundle"), fooContents)
        XCTAssertTrue(fooContents.contains("@available(*, unavailable,"), fooContents)
        ...
        let barResourceAccessor = barTarget.sources.first{ $0.basename == "resource_bundle_accessor.swift" }!
        let barContents = try fs.readFileContents(barResourceAccessor).cString
        XCTAssertTrue(barContents.contains("extension Foundation.Bundle"), barContents)
        XCTAssertTrue(barContents.contains("@available(*, unavailable,"), barContents)
    }
