...
final class PackageDescription6_5LoadingTests: PackageDescriptionLoadingTests {
    ...
    func testConditionalPluginUsage() async throws {
        ...
        guard case .plugin(let name, let package, let condition)? = manifest.targets[0].pluginUsages?.first else {
    ...
    func testEmptyPluginConditionReturnsNil() async throws {
        let content = #"""
            import PackageDescription
            let package = Package(
                name: "Foo",
        ...
        XCTAssertNoDiagnostics(observability.diagnostics)
        XCTAssertNoDiagnostics(validationDiagnostics)
        guard case .plugin(_, _, let condition)? = manifest.targets[0].pluginUsages?.first else {
            return XCTFail("expected plugin usage")
        }
        XCTAssertNil(condition)
    }

    func testHostConditionOnEveryTargetDependencyKind() async throws {
        let content = #"""
            import PackageDescription
            let package = Package(
                name: "Foo",
                traits: [.trait(name: "Feature")],
                dependencies: [
                    .package(url: "https://github.com/example/Bar.git", from: "1.0.0")
                ],
                targets: [
