...
class PIFTests: XCTestCase {
    func testEncodable() throws {
        ...
        XCTAssertEqual(projectContents["targets"]?.array, [exeTarget["signature"]!, libTarget["signature"]!])
        ...
        if let buildPhases = libTargetContents["buildPhases"]?.array, buildPhases.count == 0 {
        } else {
            XCTFail("invalid number of build configurations")
        }
    }
