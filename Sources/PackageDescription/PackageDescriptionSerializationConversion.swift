...
extension Serialization.PackageDependency {
    init(_ dependency: PackageDescription.Package.Dependency) {
        ...
        self.traits = .init(
            enabledTraits: Set(dependency.traits?.map { Serialization.PackageDependency.Traits.EnabledTrait($0) } ?? []) ,
            disableDefaultTraits: !(dependency.traits?.contains(.defaults) ?? true)
        )
    }
}


extension Serialization.PackageDependency.Traits.EnabledTrait {
...
extension Serialization.PackageDependency.Traits.EnabledTrait.Condition {
...
extension Serialization.Package {
    init(_ package: PackageDescription.Package) {
        ...
        self.defaultTraits = package.defaultTraits
