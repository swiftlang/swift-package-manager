...
fileprivate struct TripleParser {
    ...
    mutating func rematch<Value: TripleComponent>(
        _ value: inout ParsedComponent<Value>?,
        at correctIndex: Int
    ) {
        guard value == nil else { return }

        precondition(
            !isMatched.contains(correctIndex),
    ...
    private mutating func shiftComponent(
        ...
        precondition(
            !isMatched.contains(actualIndex),
        ...
        let nextIndex = unmatchedIndices[(actualIndex + 1)..<correctIndex].first ?? correctIndex
