...
* `manifests`: A non-empty map of manifests by tools version. The keys are (semantic) tools version (more on this below), while the values are:
...
* `defaultToolsVersion`: The tools version of the default manifest. The `manifests` map must contain this in its keys. 
...
The keys of the `manifests` map are tools (semantic) versions:
