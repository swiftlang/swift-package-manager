...
public struct PIFBuilderParameters {
    ...
    /// - Parameters:
    ///   - platform: The platform being built for.
    ///   - shouldCreateDylibForDynamicProducts: Whether to create dylibs for dynamic library products.
    public init(platform: PackageModel.Platform, shouldCreateDylibForDynamicProducts: Bool) {
...
final class PIFProjectBuilder {
    ...
    private let buildEnvironments: [BuildEnvironment]
    ...
    init(
        ...
        buildEnvironments = [
            BuildEnvironment(platform: parameters.platform, configuration: .debug),
            BuildEnvironment(platform: parameters.platform, configuration: .release),
        ]
    ...
    private func addMainModuleTarget(for product: ResolvedProduct) {
        ...
        for dependency in mainTarget.recursivePackageDependencies(satisfying: buildEnvironments) {
    ...
    private func addLibraryTarget(for product: ResolvedProduct) {
        ...
        let dependencies = product.recursivePackageDependencies(satisfying: buildEnvironments)
    ...
    private func addLibraryTarget(for target: ResolvedTarget) {
        ...
        for dependency in target.recursivePackageDependencies(satisfying: buildEnvironments) {
...
//        target.impartedSettings.forEach { platform, declarations in
//            let platform = PIF.BuildSettings.Platform.from(platform: platform)
//            declarations.forEach { spmDeclaration, value in
//                let declaration = PIF.BuildSettings.Declaration.from(declaration: spmDeclaration)
//                impartedSettings.platformSpecificSettings[platform]![declaration]!.append(contentsOf: value)
//            }
//        }
                ...
                if let platforms = assignment.platforms {
                    for platform in platforms {
                        for configuration in assignment.configurations {
                            ...
                            case .debug:
                                debugSettings[setting, for: platform, default: ["$(inherited)"]] += assignment.value
                            case .release:
                                releaseSettings[setting, for: platform, default: ["$(inherited)"]] += assignment.value
                            }
                        }
                    }
                } else {
                    for configuration in assignment.configurations {
                        switch configuration {
                        case .debug:
                            debugSettings[setting, default: ["$(inherited)"]] += assignment.value
                        case .release:
                            releaseSettings[setting, default: ["$(inherited)"]] += assignment.value
                        }
                    }
                }
            }
...
extension ResolvedProduct {
    ...
    public func recursivePackageDependencies(satisfying environments: [BuildEnvironment]) -> [ResolvedTarget.Dependency] {
        ...
        return try! topologicalSort(initialDependencies) { dependency in
            return dependency.packageDependencies.satisfying(any: environments)
        }.sorted()
    }
}
...
extension ResolvedTarget {
    ...
    ///     - environment: The build environment to use to filter dependencies on.
    public func recursivePackageDependencies(satisfying environments: [BuildEnvironment]) -> [ResolvedTarget.Dependency] {
        return try! topologicalSort(dependencies.satisfying(any: environments)) { dependency in
            return dependency.packageDependencies.satisfying(any: environments)
        }.sorted()
    }
}
...
extension Array where Element == ResolvedTarget.Dependency {
    ...
    /// Returns all elements that satisfy any of the input arguments.
    func satisfying(any environments: [BuildEnvironment]) -> [ResolvedTarget.Dependency] {
        filter { dependency in
            environments.contains { environment in
                dependency.satisfies(environment)
            }
        }
    }
