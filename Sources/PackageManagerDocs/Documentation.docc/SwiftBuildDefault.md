...
4. **Legacy Build System Removal** - Phase out `native` and `xcode` build systems
...
- Coverage reporting issues on some platforms (Linux, Windows)
  - **Tracking**: [swiftlang/swift-package-manager#9600](https://github.com/swiftlang/swift-package-manager/issues/9600)
- Swift Build fails to build projects with Linux SDKs genereted _by_ the SDK generator.
  - **Tracking**: TBC


### Feature gaps

- Swift Build does not provide `fuzzer` sanitizer support.
  - **Tracking**: [swiftlang/swift-package-manager#9858](https://github.com/swiftlang/swift-package-manager/issues/9858)
...
- There is an issue with Swift Build does not support the `--explicit-target-dependency-import-check` flag.
