...
SwiftPM then performs a series of validations based on user's [security configuration](<doc:UsingSwiftPackageRegistry#Security-configuration>).
- If the archive is unsigned, SwiftPM will error/prompt/warn/allow based on the `signing.onUnsigned` configuration. 
- If the archive is signed, SwiftPM will validate the signature and the signing certificate chain.
...
Since signing a package collection is optional, users will be prompted for confirmation before they can add an [unsigned collection](<doc:PackageCollectionAdd#Unsigned-package-collections>).
...
Since Package manager trusts all certificate chains under a root, depending on what roots are installed, some publishers may already be trusted implicitly and users don't need to explicitly specify each one. 
