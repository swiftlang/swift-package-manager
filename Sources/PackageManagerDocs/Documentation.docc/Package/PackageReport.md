...
```
package report [--package-path=<package-path>]
...
`swift package report` inspects the resolved package graph for products that
...
successful report (load failures still exit non-zero). This is intended for
interactive human use; CI pipelines can rely on the default behavior to
turn a finding into a failing job.

To silence the graph-load-time deprecation warnings while running a report,
`swift package report` suppresses them internally so its structured report
is the sole surface for deprecation output. A consumer target's per-target
`.treatAllWarnings(as: .error)` setting does not prevent the report from
completing.

### Output formats

`swift package report --format text` (the default) groups results into up to
...
`swift package report --format json` emits a structured document with
