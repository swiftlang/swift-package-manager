...
- **`--static-swift-stdlib`**: Stricter validation (errors instead of silently ignoring)
  ...
  - Swift Build generates an error on platforms where static libraries are not supported.
- Swift Build does not support test targets depending on other test targets. To work around this,
  create a non-test target containing the shared code, and have the test targets depend on it instead.
  Note that this extracted target should not be added as a dependency of non-test targets, as doing so
  will cause a runtime error like the one below.
    ```
    dyld[67034]: Library not loaded: @rpath/libTesting.dylib
    Referenced from: <30F1D85A-75C7-358C-B169-96E34550501C> /Users/jappleseed/.../.build/out/Products/Debug/swift-package
    Reason: tried: '/usr/lib/swift/libTesting.dylib' (no such file, not in dyld cache),
            '/Users/jappleseed/.../.build/out/Products/Debug/libTesting.dylib' (no such file),
            ...
    ```
...
- `swift sdk configure` was recently updated for the native build system. The functionality is being migrated to Swift Build.
  - [swiftlang/swift-package-manager#9229](https://github.com/swiftlang/swift-package-manager/pull/9229)
