...
Users will be prompted for confirmation before they can add an [unsigned collection](<doc:#Unsigned-package-collections>).
...
For package collections hosted on the web, publishers may ask SwiftPM to [enforce the signature requirement](<doc:Protecting-package-collections>). If a package collection is
...
On non-Apple platforms, there are no trusted root certificates by default other than those shipped with the [certificate-pinning configuration](<doc:Protecting-package-collections>). 
...
the [certificate-pinning configuration](<doc:Protecting-package-collections>), otherwise the [signature check](<doc:Signed-package-collections>) will fail. Collection publishers should make the DER-encoded 
...
To continue user must confirm their trust by passing the [`--trust-unsigned`](<doc:PackageCollectionAdd#Command-Options>) flag:
