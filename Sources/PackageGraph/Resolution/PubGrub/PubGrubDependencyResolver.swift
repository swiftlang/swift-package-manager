...
public struct PubGrubDependencyResolver {
    ...
    public func solve(constraints: [Constraint], enabledRootTraits: Set<String> = ["default"]/*traitConfiguration: TraitConfiguration = .default*/) async -> Result<[DependencyResolverBinding], Error> {
        ...
        if constraints.count == 1, let constraint = constraints.first, constraint.package.kind.isRoot {
            ...
            root = .root(package: constraint.package, enabledTraits: enabledRootTraits)
        } else {
            ...
            root = .root(
                ...
                enabledTraits: enabledRootTraits
            )
        }
