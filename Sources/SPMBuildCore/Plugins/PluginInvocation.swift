...
extension PluginModule {
    ...
    public func invoke(
        ...
        class ScriptRunnerDelegate: PluginScriptCompilerDelegate, PluginScriptRunnerDelegate {
            ...
            func handleMessage(data: Data) async throws -> Data? {
                ...
                case .defineProducts(let outputFiles):
                    try callbackQueue.sync {
                        self.invocationDelegate.pluginDefinedProducts(outputFiles: try outputFiles.map { try $0.filePath })
                    }
                    return nil
    ...
    package func invoke(
        ...
        return BuildToolPluginInvocationResult(
            ...
            productFiles: delegate.productFiles,
...
public struct BuildToolPluginInvocationResult {
    ...
    /// The list of product files produced for a custom target
    public var productFiles: [AbsolutePath]
...
public protocol PluginInvocationDelegate {
    ...
    /// Called when a plugin defines a list of product files for a custom target
    func pluginDefinedProducts(outputFiles: [AbsolutePath])
...
final class DefaultPluginInvocationDelegate: PluginInvocationDelegate {
    ...
    func pluginDefinedProducts(outputFiles: [AbsolutePath]) {
        dispatchPrecondition(condition: .onQueue(self.delegateQueue))
        self.productFiles.append(contentsOf: outputFiles)
    }
