...
public enum PluginAction {
    ...
    case externalBuild(package: ResolvedPackage)
}
...
extension PluginModule {
    ...
    public func invoke(
        ...
        do {
            ...
            case .externalBuild(let package):
                let rootPackageId = try serializer.serialize(package: package)
                let wireInput = WireInput(
                    paths: serializer.paths,
                    targets: serializer.targets,
        ...
        class ScriptRunnerDelegate: PluginScriptCompilerDelegate, PluginScriptRunnerDelegate {
            ...
            func handleMessage(data: Data, responder: @escaping (Data) -> Void) throws {
                ...
                case .defineExternalBuildCommand(let config):
        ...
        self.invoke(
            ...
            completion: {
                ...
                let invocationResult = BuildToolPluginInvocationResult(
                    ...
                    externalBuildCommands: delegate.externalBuildCommands
                )
...
extension ModulesGraph {
    public func pluginsPerModule(capability: PluginCapability) -> [ResolvedModule.ID: [ResolvedModule]] {
        var pluginsPerModule = [ResolvedModule.ID: [ResolvedModule]]()
        for module in self.allModules.sorted(by: { $0.name < $1.name }) {
            let pluginDependencies = module.pluginDependencies(capability: capability)
...
public struct BuildToolPluginInvocationResult {
    ...
    public var externalBuildCommands: [ExternalBuildCommand]
...
public protocol PluginInvocationDelegate {
    ...
    func pluginDefinedExternalBuildCommand(displayName: String?, executable: AbsolutePath, arguments: [String], environment: [String: String], workingDirectory: AbsolutePath?)
...
final class DefaultPluginInvocationDelegate: PluginInvocationDelegate {
    ...
    var externalBuildCommands: [BuildToolPluginInvocationResult.ExternalBuildCommand] = []
