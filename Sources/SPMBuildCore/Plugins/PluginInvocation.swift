...
extension PluginModule {
    ...
    public func invoke(
        ...
        class ScriptRunnerDelegate: PluginScriptCompilerDelegate, PluginScriptRunnerDelegate {
            ...
            let invocationDelegateQueue: DispatchQueue
            ...
            init(invocationDelegate: PluginInvocationDelegate, invocationDelegateQueue: DispatchQueue, observabilityScope: ObservabilityScope) {
                self.invocationDelegate = invocationDelegate
                self.invocationDelegateQueue = invocationDelegateQueue
                self.observabilityScope = observabilityScope
            }
            
            func willCompilePlugin(commandLine: [String], environment: [String: String]) {
                self.invocationDelegateQueue.async {
                    self.invocationDelegate.pluginCompilationStarted(commandLine: commandLine, environment: environment)
                }
            }
            
            func didCompilePlugin(result: PluginCompilationResult) {
                self.invocationDelegateQueue.async {
                    self.invocationDelegate.pluginCompilationEnded(result: result)
                }
            }
            
            func skippedCompilingPlugin(cachedResult: PluginCompilationResult) {
                self.invocationDelegateQueue.async {
                    self.invocationDelegate.pluginCompilationWasSkipped(cachedResult: cachedResult)
                }
            }
            ...
            func handleOutput(data: Data) {
                self.invocationDelegateQueue.async {
                    self.invocationDelegate.pluginEmittedOutput(data)
                }
            }
        ...
        let runnerDelegate = ScriptRunnerDelegate(invocationDelegate: delegate, invocationDelegateQueue: delegateQueue, observabilityScope: observabilityScope)
