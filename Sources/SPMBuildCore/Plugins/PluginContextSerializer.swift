...
internal struct PluginContextSerializer {
    ...
    var targetIDsToWireIDs: [ResolvedTarget.ID: WireInput.Target.Id] = [:]
    var products: [WireInput.Product] = []
    var productsToWireIDs: [ResolvedProduct: WireInput.Product.Id] = [:]
    var packages: [WireInput.Package] = []
    var packagesToWireIDs: [ResolvedPackage: WireInput.Package.Id] = [:]
    ...
    mutating func serialize(target: ResolvedTarget) throws -> WireInput.Target.Id? {
        ...
        if let id = targetIDsToWireIDs[target.id] { return id }
        ...
        targets.append(.init(
            ...
            info: targetInfo))
        targetIDsToWireIDs[target.id] = id
        return id
    }
    ...
    mutating func serialize(product: ResolvedProduct) throws -> WireInput.Product.Id? {
        ...
        if let id = productsToWireIDs[product] { return id }
    ...
    mutating func serialize(package: ResolvedPackage) throws -> WireInput.Package.Id {
        ...
        if let id = packagesToWireIDs[package] { return id }
