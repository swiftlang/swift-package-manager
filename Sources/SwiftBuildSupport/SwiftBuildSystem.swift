...
public final class SwiftBuildSystem: SPMBuildCore.BuildSystem {
    ...
    private func externalBuild() async throws {
        let graph = try await getPackageGraph()

        for package in graph.packages {
            for pluginUsage in package.pluginUsages {
                let pluginTarget: PluginModule
                switch pluginUsage {
                case .module(let module, conditions: let conditions):
                    guard let target = module.underlying as? PluginModule,
                          conditions.allSatisfy({ $0.satisfies(buildParameters.buildEnvironment) })
                    else {
                        continue
                    }
                    pluginTarget = target
                case .product(let product, conditions: let conditions):
                    guard let target = product.modules.compactMap({ $0.underlying as? PluginModule }).first,
                          conditions.allSatisfy({ $0.satisfies(buildParameters.buildEnvironment) })
                    else {
                        continue
                    }
                ...
                let pluginOutputDir = pluginTarget.outputDirectory(pluginWorkingDirectory: pluginConfiguration.workDirectory, package: package)

                let platformSuffix: String
                if buildParameters.buildEnvironment.platform == hostBuildParameters.buildEnvironment.platform {
                    platformSuffix = ""
                } else {
                    platformSuffix = "-" + buildParameters.buildEnvironment.platform.name
                }

                let outputDir: Basics.AbsolutePath
                switch buildParameters.configuration {
                case .debug:
                    outputDir = pluginOutputDir.appending(component: "Debug" + platformSuffix)
                case .release:
                    outputDir = pluginOutputDir.appending(component: "Release" + platformSuffix)
                }

                let triple: Triple
                if buildParameters.triple.isApple() {
                    switch buildParameters.triple.os {
                    case .macosx:
                        if let version = package.supportedPlatforms.first(where: { $0.platform == .macOS })?.version.versionString {
                            triple = try Triple(buildParameters.triple.tripleString(forPlatformVersion: version))
                        } else {
                ...
                final class Delegate: PluginInvocationDelegate {
                    var result = true
                    ...
                    func pluginCompilationEnded(result: SPMBuildCore.PluginCompilationResult) {
                    }
                    
                    func pluginCompilationWasSkipped(cachedResult: SPMBuildCore.PluginCompilationResult) {
                    }
                    
                    func pluginEmittedOutput(_ data: Data) {
                        print(String(data: data, encoding: .utf8) ?? "Bad data", terminator: "")
                    }
                ...
                let start = Date.now
                let invocationResult = try await pluginTarget.invoke(
                    action: .externalBuild(
                        package: package,
                        arguments: [],
                        triple: triple,
                        sdkPath: buildParameters.toolchain.sdkRootPath?.asURL
                    ),
                    buildEnvironment: hostBuildParameters.buildEnvironment,
                    workers: hostBuildParameters.workers,
                    scriptRunner: pluginConfiguration.scriptRunner,
                    workingDirectory: package.path,
                    outputDirectory: outputDir,
                    toolSearchDirectories: [],
                    accessibleTools: [:],
                    writableDirectories: [outputDir],
                    readOnlyDirectories: [],
                ...
                if !invocationResult || !delegate.result {
                    print("\(pluginTarget.name) failed (\(String(format: "%.2f", duration)) secs.)")
                    throw Diagnostics.fatalError
                } else {
                    print("\(pluginTarget.name) complete (\(String(format: "%.2f", duration)) secs.)")
                }
            }
        }
    }
