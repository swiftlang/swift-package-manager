...
public final class PIFBuilder {
    ...
    package func constructPIF(
        ...
        return try await memoize(to: &self.cachedPIF) {
            let rootPackages: IdentifiableSet<ResolvedPackage> = self.graph.rootPackages
            ...
            let names = rootPackages.map { $0.manifest.displayName }.joined(separator: ",")
            let workspace = PIF.Workspace(
                id: "Workspace:\(ids)",
                name: names,
                path: getCommonParentDirectory(paths: rootPackagesPaths),
...
public enum PIFGenerationError: Error {
    ...
    // case multipleRootPackagesFound(rootPackages: [ResolvedPackage])
...
extension PIFGenerationError: CustomStringConvertible {
    public var description: String {
        ...
        // case .multipleRootPackagesFound(let rootPackages):
        //     "Multiple root packages (\(rootPackages.map { $0.name }.joined(separator: ", "))) were found , making the PIF generation (root packages) ordering sensitive"
