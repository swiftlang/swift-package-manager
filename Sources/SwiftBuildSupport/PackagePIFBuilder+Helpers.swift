...
extension PackageModel.Module {
    ...
    func pifTargetGUID(suffix: TargetSuffix?) -> GUID {
        let items = [
            self.name,
            self.description,
            String(describing: self.bundleName),
            "\(self.hashValue)",
            self.path.pathString,
            self.type.rawValue,
        ].joined(separator: " - ")
        // print("Module name is: \(items)")
        // let aliases = self.moduleAliases.map {(data) in  "\(dat)=\(value)"}

        // print("Module aliases is: \(aliases)")
        return PackagePIFBuilder.targetGUID(forModuleName: self.name, suffix: suffix)
    }
}
