...
public func describe(_ prefix: AbsolutePath, _ conf: Configuration, _ graph: PackageGraph, flags: BuildFlags, toolchain: Toolchain) throws -> AbsolutePath {
    ...
    for module in graph.modules {
        ...
        case let module as ClangModule:
            ...
            let compile = try Command.compile(clangModule: module, externalModules: Set(graph.externalModules), configuration: conf, prefix: prefix, otherArgs: flags.cCompilerFlags + toolchain.clangPlatformArgs, compilerExec: toolchain.clangCompiler)
            commands += compile
            targets.append(compile, for: module)
