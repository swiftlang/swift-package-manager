...
private var cachedTempDirectory: AbsolutePath = {
    let tmpdir = ProcessInfo.environment["TMPDIR"]
    let temp = ProcessInfo.environment["TEMP"]
    let tmp = ProcessInfo.environment["TMP"]

    return AbsolutePath(tmpdir ?? temp ?? tmp ?? "/tmp/")
}()
