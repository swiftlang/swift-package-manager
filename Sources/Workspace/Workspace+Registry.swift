...
extension Workspace {
    ...
    struct RegistryAwareManifestLoader: ManifestLoaderProtocol {
        ...
        private func transformSourceControlDependenciesToRegistry(
            ...
            let transformations = await withTaskGroup(of: (PackageDependency, PackageIdentity?).self, returning: [PackageDependency: PackageIdentity].self) { group in

                for dependency in manifest.dependencies {
                    if case .sourceControl(let settings) = dependency, case .remote(let url) = settings.location {
                        group.addTask {
                            do {
                                return (dependency, try await mapRegistryIdentity(
                                    url: url,
                                    observabilityScope: observabilityScope,
                                    callbackQueue: callbackQueue
                                ))
                ...
                for await case let (dependency, identity?) in group {
            ...
            return try self.transformManifest(
                manifest: manifest,
