    ...
    public struct BinaryArtifactsManager: Cancellable {
        ...
        func download(
            ...
            if !indexFiles.isEmpty {
                ...
                for artifact in indexFiles {
                    group.enter()
                    var request = LegacyHTTPClient.Request(method: .get, url: artifact.url)
                    ...
                    self.httpClient.execute(request) { result in
                        ...
                        do {
                            ...
                            case .success(let response):
                                ...
                                guard bodyChecksum == artifact.checksum else {
                                    throw StringError(
                                        "checksum of downloaded artifact of binary target '\(artifact.targetName)' (\(bodyChecksum)) does not match checksum specified by the manifest (\(artifact.checksum))"
                                    )
                                }
                                ...
                                zipArtifacts.append(
                                    RemoteArtifact(
                                        ...
                                        url: artifact.url.deletingLastPathComponent()
                            ...
                            errors.append(error)
                            observabilityScope.emit(error: "failed retrieving '\(artifact.url)': \(error)")
                        }
                    }
                }
                                    ...
                                    self.archiver.extract(
                                        ...
                                        completion: { extractResult in
                                            ...
                                            case .failure(let error):
                                                ...
                                                observabilityScope
                                                    .emit(.remoteArtifactFailedExtraction(
                                                        artifactURL: artifact.url,
                                                        targetName: artifact.targetName,
                                                        reason: reason
                                                    ))
            ...
            // fetch and parse "artifactbundlewrapper" files, if any
            let wrapperFiles = artifacts.filter { $0.url.pathExtension.lowercased() == "artifactbundlewrapper" }
            if !wrapperFiles.isEmpty {
                let errors = ThreadSafeArrayStore<Error>()
                let jsonDecoder = JSONDecoder.makeWithDefaults()
                for artifact in wrapperFiles {
                    let destinationDirectory = artifactsDirectory
                        .appending(components: [artifact.packageRef.identity.description, artifact.targetName])
                    guard observabilityScope
                        .trap({ try fileSystem.createDirectory(destinationDirectory, recursive: true) })
                    ...
                    request.options.validResponseCodes = [200]
                    request.options.authorizationProvider = self.authorizationProvider?.httpAuthorizationHeader(for:)
                    self.httpClient.execute(request) { metadataResult in
                        defer { group.leave() }
                        ...
                        do {
                            ...
                            case .success(let response):
                                ...
                                let wrapperMetadata = try jsonDecoder.decode(ArchiveWrapperFile.self, from: body)
                                guard wrapperMetadata.supportedTriples.contains(self.hostToolchain.triple) else {
                                    // FIXME: error message
                                    throw StringError(
                                        "No supported archive was found for '\(self.hostToolchain.triple.tripleString)'"
                                    )
                                ...
                                let downloadStart: DispatchTime = .now()
                                self.delegate?.willDownloadBinaryArtifact(from: wrapperMetadata.url.absoluteString)
                                observabilityScope.emit(debug: "downloading \(artifact.url) to \(archivePath)")
                                self.httpClient.execute(request) { downloadResult in
                                    defer { group.leave() }
                                    ...
                                    do {
                                        ...
                                        case .success:
                                            ...
                                            self.archiver.validate(path: archivePath, completion: { validationResult in
                                                ...
                                                case .success(let valid):
                                                    ...
                                                    guard archiveChecksum == wrapperMetadata.checksum else {
                                                        observabilityScope.emit(.artifactInvalidChecksum(
                                                            targetName: artifact.targetName,
                                                            expectedChecksum: wrapperMetadata.checksum,
                                                            actualChecksum: archiveChecksum
                                                        ))
                                                        observabilityScope
                                                            .trap { try self.fileSystem.removeFileTree(archivePath) }
                                                        return
                                                    }

                                                    guard let tempExtractionDirectory = observabilityScope
                                                        .trap({ () -> AbsolutePath in
                                                            let path = artifactsDirectory.appending(
                                                                components: "extract",
                                                                artifact.packageRef.identity.description,
                                                                artifact.targetName,
                                                    ...
                                                    observabilityScope
                                                        .emit(
                                                            debug: "extracting \(archivePath) to \(tempExtractionDirectory)"
                                                        )
                                                    self.archiver.extract(
                                                        from: archivePath,
                                                        to: tempExtractionDirectory,
                                                        completion: { extractResult in
                                                            ...
                                                            case .success:
                                                                observabilityScope.trap {
                                                                    try self.fileSystem.withLock(
                                                                        ...
                                                                        if try self.fileSystem.shouldStripFirstLevel(
                                                                            archiveDirectory: tempExtractionDirectory,
                                                                            acceptableExtensions: BinaryTarget.Kind
                                                                                .allCases
                                                                                .map(\.fileExtension)
                                                                        ) {
                                                                            observabilityScope
                                                                        ...
                                                                        for file in content {
                                                                            ...
                                                                            if self.fileSystem.exists(destination) {
                                                                                try self.fileSystem
                                                                                    .removeFileTree(destination)
                                                                            }
                                                                            try self.fileSystem.copy(
                                                                                from: source,
                                                                                to: destination
                                                                            )
                                                                        }
                                                                    ...
                                                                    // remove temp location
                                                                    try self.fileSystem
                                                                        .removeFileTree(tempExtractionDirectory)
                                                                }

                                                                // derive concrete artifact path and type
                                                                let artifactPath = destinationDirectory
                                                                    .appending(wrapperMetadata.path)
                                                            ...
                                                            case .failure(let error):
                                                                let reason = (error as? LocalizedError)?
                                                                    .errorDescription ?? "\(error)"
                                                                observabilityScope
                                                                    .emit(
                                                                        .remoteArtifactFailedExtraction(
                                        ...
                                        errors.append(error)
                                        observabilityScope
                                            .emit(error: "failed retrieving '\(wrapperMetadata.url)': \(error)")
                                    }
                                }
                            }
                        } catch {
...
extension Workspace.BinaryArtifactsManager {
    struct ArchiveWrapperFile: Decodable {
        let schemaVersion: String
        let url: URL
        let checksum: String
