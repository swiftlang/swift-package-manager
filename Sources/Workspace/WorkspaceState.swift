...
public final class ManagedArtifact {
    ...
    public enum Source: Equatable {
        ...
        /// The path of the artifact on disk, relative to the workspace artifacts path.
        case remote(subpath: RelativePath, checksum: String)

        /// The artifact is available locally.
    ...
    public static func remote(
        ...
        return ManagedArtifact(
            ...
            source: .remote(subpath: subpath, checksum: checksum)
        )
    }
...
public final class WorkspaceState: SimplePersistanceProtocol {
    ...
    init(dataPath: AbsolutePath, fileSystem: FileSystem) {
        ...
        self.persistence = SimplePersistence(
            ...
            statePaths: [
                statePath,
                dataPath.appending(component: "dependencies-state.json"),
            ]
        )
    ...
    public func restore(from json: JSON) throws {
        ...
        artifacts = try ManagedArtifacts(json: json.get("dependencies"))
    }
...
public final class ManagedArtifacts {
    ...
    private var values: AnySequence<ManagedArtifact> {
        AnySequence(artifactMap.values.lazy.flatMap({ $0.values }))
    }
...
extension ManagedArtifact.Source: JSONMappable, JSONSerializable, CustomStringConvertible {
    public func toJSON() -> JSON {
        ...
        case .remote(let subpath, let checksum):
            return .init([
                "type": "remote",
    ...
    public var description: String {
        ...
        case .local(let path):
            return "local(\(path))"
        case .remote(let subpath, let checksum):
            return "remote(\(subpath), \(checksum))"
        }
    }
}

extension ManagedDependencies: JSONMappable, JSONSerializable, Sequence, CustomStringConvertible {
    public convenience init(json: JSON) throws {
    ...
    public func makeIterator() -> AnyIterator<ManagedDependency> {
        AnyIterator(dependencyMap.values.makeIterator())
    }
...
extension ManagedArtifacts: JSONMappable, JSONSerializable, Sequence, CustomStringConvertible {
    public convenience init(json: JSON) throws {
    ...
    public func makeIterator() -> AnyIterator<ManagedArtifact> {
        AnyIterator(values.makeIterator())
    }

    public var description: String {
        "<ManagedArtifacts: \(Array(values))>"
    }
}
