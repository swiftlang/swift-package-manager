...
public class RegistryPackageContainer: PackageContainer {
    ...
    private func loadUnmemoizedManifest(version: Version) async throws -> Manifest {
        let result = try await self.getAvailableManifestsFilesystem(version: version)
    ...
    private func getAvailableManifestsFilesystem(
        version: Version
    ) async throws -> (manifests: [String: (toolsVersion: ToolsVersion, content: String?)], fileSystem: FileSystem) {
        // try cached first
