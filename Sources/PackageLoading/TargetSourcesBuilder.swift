        ...
        generatedFiles.forEach { absPath in
            ...
            case .compile:
                if absPath.extension == "swift" {
                    ...
                    observabilityScope.emit(warning: "C source file generation requires tools version >= 6.3: \(absPath)")
                }
            ...
            case .header:
                if toolsVersion >= .v6_3 {
                    ...
                    observabilityScope.emit(warning: "C header file generation requires tools version >= 6.3: \(absPath)")
                }
            case .modulemap:
                if toolsVersion >= .v6_3 {
                    if module is ClangModule {
                    ...
                    observabilityScope.emit(warning: "Module map generation requires tools version >= 6.3: \(absPath)")
                }
            case .apinotes:
                if toolsVersion >= .v6_3 {
                    if module is ClangModule {
                    ...
                    observabilityScope.emit(warning: "API notes generation requires tools version >= 6.3: \(absPath)")
                }
            case .ignored, .none:
                break
            }
