...
public struct PackageBuilder {
    ...
    private func constructModules() throws -> [Module] {
        ...
        // If atleast one module in the source directory, make sure there are no loose source files in the sources
        // directory.
        ...
        if potentialModulePaths.isEmpty {
            // There are no directories that look like modules, so try to create a module for the source directory
    ...
    /// Private function that checks whether a module name is valid.  This method doesn't return anything, but rather,
...
private struct PotentialModule: Hashable {
    ...
    public var basename: String {
        ...
        let endIndex = name.index(name.endIndex, offsetBy: -Module.testModuleNameSuffix.characters.count)
