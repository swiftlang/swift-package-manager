...
package struct CoverageFormatOutput: Encodable {
    ...
    package mutating func addFormat(_ format: CoverageFormat, path: AbsolutePath) throws {
        guard !_underlying.keys.contains(format) else {
            throw CoverageFormatError.duplicateEntry(format: format)
        }
    ...
    package func getPath(for format: CoverageFormat) throws -> AbsolutePath {
        guard let path = _underlying[format] else {
            throw CoverageFormatError.missingOutputPath(format: format)
        }
        return path
    }
...
package enum CoverageFormatError: Error, Equatable {
    case duplicateEntry(format: CoverageFormat)
    case missingOutputPath(format: CoverageFormat)
}

extension CoverageFormatError: CustomStringConvertible {
    package var description: String {
        switch self {
            case .duplicateEntry(let format):
               "Coverage format '\(format.rawValue)' already exists."
            case .missingOutputPath(let format):
                "Missing coverage format output path for '\(format.rawValue)'"
        }
    }
}
