...
struct Report: AsyncSwiftCommand {
    ...
    func run(_ swiftCommandState: SwiftCommandState) async throws {
        // Report produces its own structured deprecation report — suppress the
        ...
        switch self.format {
        case .text:
            print(renderPackageReportAsText(report))
        case .json:
    ...
    enum ReportFormat: String, RawRepresentable, CustomStringConvertible, ExpressibleByArgument, CaseIterable {
        case text, json
...
/// Filter applied to the deprecation report.
///
...
struct PackageReport: Codable, Equatable {
    ...
    /// `true` iff any product in the report should fail the command (any
...
/// Walks the resolved graph and produces a report of deprecated products.
///
/// - Parameters:
///   - graph: The resolved package graph to inspect.
///   - includeTransitive: Filter mode. See `IncludeTransitive` for semantics.
/// - Returns: A `PackageReport` with products sorted by `(package, product)`
///   for deterministic output.
func buildPackageReport(
    graph: ModulesGraph,
    ...
    for product in graph.allProducts {
        ...
        guard reportProductTypeString(product.type) != nil else { continue }
        let key = ProductKey(
    ...
    for (key, product) in deprecatedProducts {
        guard let typeString = reportProductTypeString(product.type) else { continue }
    ...
    return PackageReport(deprecated: .init(products: entries))
}
        ...
        for dependency in frame.module.dependencies {
            ...
            case .product(let dependeeProduct, _):
                ...
                if deprecatedProducts[key] != nil {
                    ...
                    // deprecated product for reporting purposes.)
...
/// sequences. Those are indistinguishable in the report's product-oriented
...
/// Renders a `PackageReport` as human-readable text, grouped into three
...
func renderPackageReportAsText(_ report: PackageReport) -> String {
...
/// Formats a single report section: header line followed by per-package
...
/// Maps a `ProductType` to the schema-legal string used in the report JSON
...
func reportProductTypeString(_ type: ProductType) -> String? {
    switch type {
    case .executable:
        return "executable"
    case .library:
